...
final readonly class FfprobeAnalyzer
{
    ...
    public function inspect(string $mountPath,string $candidate):array
    {
        $relative=$this->paths->relative($mountPath,$candidate);$binary=(string)($_ENV['FFPROBE_PATH']??'/usr/local/bin/ffprobe');if(!is_file($binary))throw new RuntimeException('FFprobe no está instalado en la ruta configurada.');$real=realpath($candidate);$pipes=[];$process=proc_open([$binary,'-v','error','-show_format','-show_streams','-of','json',$real],[1=>['pipe','w'],2=>['pipe','w']],$pipes,null,null,['bypass_shell'=>true]);if(!is_resource($process))throw new RuntimeException('No fue posible iniciar FFprobe.');stream_set_blocking($pipes[1],false);stream_set_blocking($pipes[2],false);$stdout='';$stderr='';$started=microtime(true);$timeout=max(5,min(120,(int)($_ENV['FFPROBE_TIMEOUT']??30)));do{$stdout.=stream_get_contents($pipes[1]);$stderr.=stream_get_contents($pipes[2]);$status=proc_get_status($process);if(!$status['running'])break;if(microtime(true)-$started>$timeout){proc_terminate($process,9);throw new RuntimeException('FFprobe excedió el tiempo permitido.');}usleep(20000);}while(true);$stdout.=stream_get_contents($pipes[1]);$stderr.=stream_get_contents($pipes[2]);fclose($pipes[1]);fclose($pipes[2]);$exit=proc_close($process);if($exit===-1)$exit=(int)($status['exitcode']??-1);if($exit!==0)throw new RuntimeException('FFprobe rechazó el archivo: '.substr(trim($stderr),0,300));return ['relative_path'=>$relative,'size_bytes'=>filesize($real),'probe'=>json_decode($stdout,true,512,JSON_THROW_ON_ERROR)];
    }
