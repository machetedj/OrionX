...
{
 public function __construct(private TaskQueueService $queue,private LibraryScanner $scanner,private TmdbService $tmdb,private ServerHealthService $health,private CertificateIssuer $certificates,private SshProvisioner $provisioner,private PDO $db){}
 ...
 private function handle(array $job):array{$p=$job['payload'];return match($job['type']){'import_movies'=>$this->scanner->scan((int)($p['library_id']??0),'movie'),'import_series'=>$this->scanner->scan((int)($p['library_id']??0),'series'),'query_tmdb'=>['tmdb'=>$this->tmdb->details((string)($p['type']??'movie'),(int)($p['tmdb_id']??0),(string)($p['language']??'es-ES'))],'cleanup_sessions'=>$this->cleanupSessions(),'rotate_logs'=>$this->rotateLogs(),'sync_balancers'=>['isolated'=>$this->health->isolateStale((int)($p['heartbeat_ttl']??90))],'issue_certificate'=>$this->certificates->issueMain((int)($p['request_id']??0)),'provision_balancer','sync_balancer','update_balancer'=>$this->provisioner->deploy((int)($p['deployment_id']??0)),default=>throw new RuntimeException('Handler aún no configurado para '.$job['type'])};}
