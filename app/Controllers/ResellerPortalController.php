...
final readonly class ResellerPortalController
{
 ...
 public function index(Request $r):void{$profile=$this->repo->byUser(Auth::id()??0);if(!$profile)Response::redirect('/reseller/login');$this->view->render('reseller/dashboard',['title'=>$profile['brand_name']?:'Panel de revendedor','profile'=>$profile,'accounts'=>$this->repo->accounts((int)$profile['id']),'movements'=>$this->repo->movements((int)$profile['id']),'packages'=>$this->repo->packages((int)$profile['id']),'csrf'=>$this->csrf]);}
