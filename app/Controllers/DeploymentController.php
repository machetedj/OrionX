...
final readonly class DeploymentController
{
 ...
 public function fingerprint(Request $r):void{$this->allow();try{Response::json(['ok'=>true,'fingerprint'=>$this->service->fingerprint($r->int('server_id'),$r->int('port')?:35222)]);}catch(Throwable $e){Response::json(['ok'=>false,'error'=>$e->getMessage()],422);}}
 public function fingerprintHost(Request $r):void{$this->allow();try{Response::json(['ok'=>true,'fingerprint'=>$this->service->fingerprintHost($r->string('server_ip'),$r->int('port')?:35222)]);}catch(Throwable $e){Response::json(['ok'=>false,'error'=>$e->getMessage()],422);}}
 public function install(Request $r):void{$this->allow();$this->attempt(fn()=> $this->service->installNew($r->input));}
 public function credentials(Request $r):void{$this->allow();$this->attempt(fn()=> $this->service->saveCredentials($r->int('server_id'),$r->int('port')?:35222,$r->string('ssh_user'),$r->string('fingerprint'),(string)($r->input['private_key']??''),(string)($r->input['ssh_password']??'')));}
