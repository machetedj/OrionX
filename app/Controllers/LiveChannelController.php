...
use App\Core\{Csrf,Request,Response,View};use App\Repositories\LiveChannelRepository;use App\Security\Auth;use App\Services\{LiveChannelService,StreamMonitorService};use Throwable;
final readonly class LiveChannelController
{
 public function __construct(private View $view,private LiveChannelRepository $repo,private LiveChannelService $service,private StreamMonitorService $monitor,private Csrf $csrf){}
 ...
 public function restart(Request $r):void{$this->allow();$this->attempt(fn()=> $this->monitor->restart($r->int('channel_id')));}
