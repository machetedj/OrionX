...
use App\Core\{Csrf,Request,Response,View};use App\Repositories\MediaLibraryRepository;use App\Security\Auth;use App\Services\{MediaLibraryService,RemoteMediaService};use Throwable;
final readonly class MediaLibraryController
{
 public function __construct(private View $view,private MediaLibraryRepository $repo,private MediaLibraryService $service,private RemoteMediaService $remote,private Csrf $csrf){}
 public function index(Request $r):void{$this->allow();$this->view->render('libraries/index',['title'=>'Bibliotecas y carga masiva','libraries'=>$this->repo->libraries(),'servers'=>$this->repo->servers(),'jobs'=>$this->repo->jobs(),'conflicts'=>$this->repo->conflicts(),'remoteRuns'=>$this->repo->remoteRuns(),'links'=>$this->repo->links(),'csrf'=>$this->csrf]);}
