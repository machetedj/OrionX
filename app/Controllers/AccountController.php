...
final readonly class AccountController
{
    ...
    public function index(Request $request): void { $this->allow('accounts.view');$filters=['q'=>$request->string('q'),'status'=>$request->string('status'),'online'=>$request->string('online'),'owner'=>$request->int('owner'),'page'=>max(1,$request->int('page')),'per_page'=>$request->int('per_page')];$result=$this->accounts->all($filters);$this->view->render('accounts/index',['title'=>'Líneas','accounts'=>$result['items'],'pagination'=>$result['pagination'],'filters'=>$filters,'summary'=>$this->accounts->summary(),'owners'=>$this->accounts->owners(),'packages'=>$this->accounts->packages(),'csrf'=>$this->csrf]); }
